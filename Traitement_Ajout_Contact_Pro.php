<?php
	 ...
	 if($reponse=='Accepter'){
		 	...
		 	$sql = "INSERT INTO etrecontactpro (numero_utilisateur1,numero_utilisateur2) VALUES ('$numero_utilisateur','$numero_emetteur')" ;
		 	echo "<br> SQL : $sql"; 
			if($selection = mysqli_query($connexion,$sql) === TRUE) echo "success"; 
		}
		$sql = "DELETE FROM notification WHERE ((description='Demande de contact pro') AND (numero_emetteur='$numero_emetteur') AND (numero_recepteur='$numero_utilisateur'))" ; 
		echo "<br> SQL : $sql"; 
		if($selection = mysqli_query($connexion,$sql) === TRUE) echo "success"; 
